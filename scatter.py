...
# 经过计算，结果为两部分之和： A1^3 + 3*A1*A5^2 ，其中 A1 为动量空间态密度积分（
# 求和），A5 为动量空间态密度的傅里叶变换，即乘上 exp^(i(kx*x + ky*y)) 然后求和
# ，由于变换前的对称性，A5 的虚部可略去，若为磁性散射，则系数 3 改为 -1。
A1 = np.sum(dos)
A2 = np.power(A1, 3)
# 计算第一部分。

length = 1000 # 为了让结果更精细，实空间的格点选取得更密集。
